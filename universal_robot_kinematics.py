...
def file2array(path, delimiter=','):     # delimiter是数据分隔符
    ...
    fp.close()
    row_list = string.splitlines(True)  # splitlines默认参数是‘\n’
    line_number = int(len(row_list))
    data_list = [[np.array(i) for i in row.strip().split(delimiter)] for row in row_list]
    return np.array(data_list), line_number
...
""" def file2array(path):
  ...
  for line in lines:
    row = line.split(', ')
    matrix.append(row)
  return matrix, lines_number """



if __name__ == "__main__":
  c = [0]
  data, line_number = file2array('trajx_28.txt')
  data = np.array(data)
  xyz = np.zeros([line_number, 3])
  for i in range(line_number):
    joint_angle = data[i].reshape(6, 1)
    location = HTrans(joint_angle, c)
    xyz[i, 0] = location[0, 3]
    xyz[i, 1] = location[1, 3]
    xyz[i, 2] = location[2, 3]
  np.savetxt("x.txt", xyz, fmt = '%.18f, %.18f, %.18f', delimiter = '\n')
  print(xyz)